...
type TokenHandler = fn(&mut PushBackIterator<Peekable<Chars>>) -> Option<Token>;
...
pub fn tokenize(compilation_unit: &str) -> Vec<Token> {
    ...
    let handlers: Vec<TokenHandler> = vec![
        handle_whitespace,
        handle_alpha,
        handle_number,
        handle_operator,
...
fn handle_whitespace(iter: &mut PushBackIterator<Peekable<Chars>>) -> Option<Token> {
    ...
    handled.then(|| Token::new(TokenType::Whitespace, " ".to_string(), false))
}
...
fn is_keyword(token: &str) -> bool {
    matches!(
        token,
        "class" | "public" | "static" | "void" | "final" | "import" | "if" | "else" | "return" | "while"
    )
}
